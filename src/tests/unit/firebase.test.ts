              ...
              const unsubscribe2 = FirebaseChat.subscribeToChat(
                ...
                async (messages) => {
                  ...
                  if (chat2Updates === 1) {
                    ...
                    expect(chat1Messages[0].receiver).toBe(
                      receiver1Id.toString()
                    );

                    expect(chat2Messages).toHaveLength(1);
                    expect(chat2Messages[0].content).toBe("Hello receiver2");
                    expect(chat2Messages[0].receiver).toBe(
                      receiver2Id.toString()
                    );
                    ...
                    setTimeout(async () => {
                      ...
                      expect(
                        chat1Messages.every(
                          (m) => m.receiver === receiver1Id.toString()
                        )
                      ).toBe(true);
                      ...
                      expect(
                        chat2Messages.every(
                          (m) => m.receiver === receiver2Id.toString()
                        )
                      ).toBe(true);
  ...
  it("should handle rapid chat switching without memory leaks", async () => {
    ...
    await new Promise<void>((resolve) => {
      ...
      chatIds.forEach((chatId) => {
        ...
        const unsubscribe = FirebaseChat.subscribeToChat(
          chatId.toString(),
          () => {
